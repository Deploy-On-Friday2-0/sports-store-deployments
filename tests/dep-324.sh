...
grep -Fq -- 'kind: Ingress' "$rendered" || fail "no Ingress rendered for the production-ingress release"
grep -Fq -- 'ingressClassName: alb' "$rendered" || fail "rendered ingress class is not alb"
grep -Fq -- 'name: sports-store-gateway' "$rendered" || fail "ingress is not named sports-store-gateway"

path_count="$(grep -Fc -- 'path: /api/' "$rendered" || true)"
[[ "${path_count:-0}" == "5" ]] || fail "expected exactly 5 /api/* rules, found ${path_count:-0}"

for backend in auth catalog cart order payment; do
  grep -Fq -- "name: $backend" "$rendered" || fail "rendered ingress has no backend for $backend"
done
for banned in gateway frontend; do
  if grep -Fq -- "name: $banned" "$rendered"; then
    fail "rendered ingress routes to the $banned workload - the gateway must be bypassed"
  fi
done

grep -Fq -- 'alb.ingress.kubernetes.io/actions.catalog' "$rendered" || fail "missing ALB actions.catalog annotation (Argo Rollouts canary weight)"
grep -Fq -- 'alb.ingress.kubernetes.io/actions.order' "$rendered" || fail "missing ALB actions.order annotation (Argo Rollouts canary weight)"
grep -Fq -- 'alb.ingress.kubernetes.io/healthcheck-path: /health' "$rendered" || fail "ALB healthcheck path is not /health"
