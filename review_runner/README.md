...
1. `.github/workflows/helm.yml` runs every deterministic Pull Request check: branch naming, Helm lint, and Helm unit tests.
2. `.github/workflows/ai-review-after-ci.yml` runs from the default branch after `Helm validation` completes. Its job exists only when the completed run is a successful Pull Request run with resolved PR metadata.
...
Disable reviews by disabling `.github/workflows/ai-review-after-ci.yml` in Actions or removing its `workflow_run` trigger on the default branch. Do not weaken the success condition, add `always()`, move the OpenRouter secret into `helm.yml`, or use `secrets: inherit`.
...
```bash
python -m pip install -r review_runner/requirements-test.txt
pytest tests/test_ai_review_workflows.py tests/test_github_comment.py tests/test_github_review.py tests/test_openrouter_provider.py tests/test_review_runner.py
python -m pip check
```
...
3. Failing branch-name validation, Helm lint, or Helm unit tests leaves the AI workflow unstarted and does not consume provider quota.
